...
void URPGAttributeSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	//レプリケートしたいものすべてに必要な正常性を登録すること
	//無条件で登録される、
	//REPNOTIFY_Alwaysは常に値がサーバーに設定されている場合はそれを複製し、クライアント上で複製することを意味する
	//REPNOTIFY_OnChangedはサーバー上でヘルスの値が変更されていない場合、レプリケーションを行わない
	//
...
void URPGAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	//GameplayEffect実行時に実際に値をクランプされた値に設定し直す。
	...
	if (Data.EvaluatedData.Attribute == GetIncomingDamageAttribute())
	{
		...
		if (LocalIncomingDamage > 0.f)
		{
			...
			{
				//Effects_HitReactタグがついているAbilityを持っている場合はそれをアクティブにする
